...
unsigned long sendFile(const char* fileName)
{
	...
	while(!feof(fp))
	{
		...
		cout << "numBytesSent = " << numBytesSent << endl;
	...
	message sndMsg2;
	sndMsg2.size = 0;
	sndMsg2.mtype = SENDER_DATA_TYPE;

	if(msgsnd(msqid, &sndMsg2, sizeof(message) - sizeof(long), 0) < 0)
	{
		perror("msgsnd");
		exit(-1);
	}
...
void sendFileName(const char* fileName)
{
	...
	my_File.mtype = FILE_NAME_TRANSFER_TYPE;
	/* TODO: Set the file name in the message */
	strncpy(my_File.fileName, fileName, fileNameSize+1);
	//my_File.fileName = "keyfile.txt";
	/* TODO: Send the message using msgsnd */
	if(msgsnd(msqid, &my_File, sizeof(fileNameMsg) - sizeof(long), 0) < 0)
	{
		perror("msgsnd");
		exit(-1);
	}
