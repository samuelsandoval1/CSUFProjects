...
std::istream &operator>>(std::istream &stream, Book &book)
{
  ...
  stream >> std::quoted(workingItem._isbn) >> delimiter >> std::quoted(workingItem._title) >> delimiter >> std::quoted(workingItem._author) >> delimiter >> workingItem._price;
 ...
 ************************/
// TO DO
BookList::BookList(const std::size_t &newSize) : _capacity(newSize), _books_array_size(0), _bookArray(new Book[newSize])
{
}
