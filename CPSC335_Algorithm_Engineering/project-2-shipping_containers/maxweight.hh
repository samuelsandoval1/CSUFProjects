...
{
	std::unique_ptr<CargoVector> result(new CargoVector);
	for(auto& cargo : source){
		if(cargo->weight() >= min_weight && cargo->weight() <= max_weight && result->size() < total_size){
			result->push_back(cargo);
		}
	}
	return result;
}
...
{
	CargoVector currentGoods = goods;
	double result_volume = 0;
	int count, index;
	std::unique_ptr<CargoVector> result (new CargoVector);

	while(!currentGoods.empty()){
		count = 0;
		index = 0;
		std::shared_ptr<CargoItem> itemObj = nullptr;

		for(auto& cargo : currentGoods){
			if(itemObj == nullptr || (cargo->weight()/cargo->volume() > itemObj->weight()/itemObj->volume())){
				itemObj = cargo;
				index = count;
			}
			++count;
		}
		currentGoods.erase(currentGoods.begin()+index);

		if(result_volume + itemObj->volume() <= total_volume){
			result->push_back(itemObj);
			result_volume += itemObj->volume();
		}
	}
	return result;
}
